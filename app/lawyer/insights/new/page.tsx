...
import { InsightCategory } from '@/lib/database.types'
...
export default function NewInsightPage() {
    ...
    try {
      ...
      if (response.ok && result.success) {
        toast({
          title: '✅ Success',
          description: publish 
            ? `Insight published successfully! It's now visible to all users.` 
            : 'Insight saved as draft. You can publish it later.'
        })
        ...
        const errorMessage = result.error || 'Failed to save insight'
        const errorDetails = result.details
        
        if (errorMessage.includes('subscription')) {
          toast({
            title: '❌ Subscription Required',
            description: errorDetails || 'You need an active subscription to publish insights. You can save as draft instead.',
            variant: 'destructive'
          })
        } else if (errorMessage.includes('active')) {
          toast({
            title: '❌ Account Not Active',
            description: errorDetails || 'Your lawyer profile must be active to create insights.',
            variant: 'destructive'
          })
  ...
  return (
    <div className="container max-w-4xl py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Create Legal Insight</h1>
        <p className="text-muted-foreground mt-2">
          Share your legal knowledge to educate potential clients
        </p>
      </div>
      ...
      <Alert className="mb-6 border-amber-200 bg-amber-50">
        ...
        <AlertDescription className="text-amber-900">
          ...
          Avoid commands, promises, or specific recommendations to individuals.
        </AlertDescription>
      </Alert>
      ...
      <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm ${title.length < 10 ? 'text-destructive font-semibold' : 'text-muted-foreground'}`}>
                  {title.length}/200
                </span>
              </div>
              <Input
                ...
                maxLength={200}
                className={title.length > 0 && title.length < 10 ? 'border-destructive' : ''}
              />
              {title.length > 0 && title.length < 10 && (
                <p className="text-sm text-destructive">
                  Title must be at least 10 characters (need {10 - title.length} more)
                </p>
              )}
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Write your legal insight (100-5000 characters)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm ${content.length < 100 ? 'text-destructive font-semibold' : 'text-muted-foreground'}`}>
                  {content.length}/5000
                </span>
              </div>
              <Textarea
                ...
                placeholder="Share your legal knowledge in a clear, informational way..."
                rows={15}
                maxLength={5000}
                className={content.length > 0 && content.length < 100 ? 'border-destructive' : ''}
              />
              {content.length > 0 && content.length < 100 && (
                <p className="text-sm text-destructive">
                  Content must be at least 100 characters (need {100 - content.length} more)
                </p>
              )}
            </div>
        ...
        <div className="flex gap-4">
          ...
          <Button
            ...
            disabled={loading}
          >
            {loading ? 'Saving...' : isPublished ? 'Publish' : 'Save as Draft'}
          </Button>
        </div>
      </div>
    </div>
