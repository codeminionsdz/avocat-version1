...
export default function LawyerChatPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const clientName = consultation.clientName || "عميل غير معروف"
  const clientPhone = consultation.clientPhone || ""
  const clientInitials = clientName.split(" ").map((n) => n[0]).join("").toUpperCase() || "UC"
      ...
      <div className="border-b bg-background sticky top-0 z-10">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <h2 className="font-semibold text-foreground">{clientName}</h2>
